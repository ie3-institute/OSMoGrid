...
class GraphControllerTest extends Specification {
    ...
    def "calculated perpendicular distance matrix test"() {
        ...
        def actual = ((DistanceWeightedGraph<OsmogridNode>) graphController["fullGraph"]).
                vertexSet().
                parallelStream().
                filter({node -> Objects.nonNull(node.getHouseConnectionPoint())}).
                collect(
                        Collectors.toMap(
                                {node -> ((OsmogridNode) node).id},
                                {node -> Pair.of(((OsmogridNode) node).latlon, ((OsmogridNode) node).houseConnectionPoint)})
                )
