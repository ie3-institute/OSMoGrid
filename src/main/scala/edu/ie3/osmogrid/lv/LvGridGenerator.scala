...
object LvGridGenerator {
  ...
  /**
    ...
    */
  private def buildGrid(graphModel: AsSubgraph[OsmGridNode, DistanceWeightedOsmEdge]) =  {
      val vRated = Quantities.getQuantity(OsmoGridConfig.Grid.ratedVoltage, PowerSystemUnits.KILOVOLT)
      val vTarget = Quantities.getQuantity(1d, PowerSystemUnits.PU)
      val voltLvl: VoltageLevel = Try(GermanVoltageLevelUtils.parse(OsmoGridConfig.Grid.voltageLevel, vRated)) match {
          case Success(voltageLevel: VoltageLevel) => voltLvl
          case Failure(e) => LvGridGenerator.logger.error("Could not set voltage level from config file. Continue with {}", voltLvl, e)
        ...
        for (osmGridNode <- subgraph.vertexSet) {
          if (osmGridNode.getLoad != null) {
            ...
            if (OsmoGridConfig.Grid.considerHouseConnectionPoints) {
              ...
              val eConsAnnual: ComparableQuantity[Energy] = Quantities.getQuantity(
                osmGridNode.getLoad.toSystemUnit.getValue.doubleValue,
                PowerSystemUnits.KILOWATTHOUR)
              val loadInput: LoadInput = new LoadInput(
                UUID.randomUUID, "Load " + loadIdCounter,
                houseConnectionPoint,
                CosPhiFixed.CONSTANT_CHARACTERISTIC,
                BdewLoadProfile.H0,
                false,
                eConsAnnual,
                osmGridNode.getLoad.to(PowerSystemUnits.KILOVOLTAMPERE).asInstanceOf[ComparableQuantity[Power]],
                1d)
              ...
              val length: ComparableQuantity[Length] = GeoUtils.calcHaversine( //TODO: = weightededge of Graph?
              ...
              val lineInput: LineInput = new LineInput(UUID.randomUUID, "Line " + {
                lineIdCounter += 1
                lineIdCounter - 1
              },
                nodeInput,
                ...
                lineType,
                length,
                GridAndGeoUtils.buildSafeLineStringBetweenNodes(nodeInput, houseConnectionPoint),
              ...
              val eConsAnnual: ComparableQuantity[Energy] = Quantities.getQuantity(osmGridNode.getLoad.toSystemUnit.getValue.doubleValue, PowerSystemUnits.KILOWATTHOUR)
              val loadInput: LoadInput = new LoadInput(UUID.randomUUID, "Load " + {
                loadIdCounter += 1
                loadIdCounter - 1
              },
                nodeInput,
            ...
            if (subgraph.degreeOf(osmGridNode) > 2) { // If the node is an intersection, create a NodeInput
              val nodeInput: NodeInput = new NodeInput(UUID.randomUUID, "Node " + {
                  nodeIdCounter += 1
                  nodeIdCounter - 1
                },
                  vTarget,
            ...
            lineBuilder.initialize(subgraph,
              geoGridNodesMap,
              lineIdCounter)
            lineBuilder.start()
  ...
  private def buildGridContainer(nodes: util.Set[NodeInput], lines: util.Set[LineInput], loads: util.Set[LoadInput]) =  {
    ...
    new JointGridContainer(OsmoGridConfig.Runtime.name, rawGridElements, systemParticipants, graphicElements)
  }
  ...
  private def loadTypes(): Unit = {
    ...
    val sourceFormat: TypeSourceFormat = TypeSourceFormat.valueOf(OsmoGridConfig.Io.typeSourceFormat)
    if (OsmoGridConfig.Io.readTypes) {
      if (sourceFormat eq TypeSourceFormat.CSV) {
        typeSource = new CsvTypeSource(";", "", new FileNamingStrategy)
        lineType = typeSource.getLineTypes.stream.filter((lineTypeInput: LineTypeInput) => lineTypeInput.getId == OsmoGridConfig.Grid.lineType).findAny.orElse(LvGridGenerator.builtDefaultLineType())
      }
      else {
        LvGridGenerator.logger.error("Invalid TypeSource in config file. Use standard LineTypeInput")
  ...
  /**
    * Calls the methods that are necessary for building the complete electrical grid out of a GraphModel.
    *
    * @param graphModel
    * GraphModel that was generated in the GraphController.
    * @return
    * Returns the complete electrical grid as a List of GridInputModels (one for each subnet).
    */
  def generateGrid(graphModel: List[AsSubgraph[OsmGridNode, DistanceWeightedOsmEdge]]): JointGridContainer = {
    ...
    for (subGrid <- gridModel.getSubGridTopologyGraph.vertexSet.asScala) {
      val nodeCodeMap: OneToOneMap[String, Integer] = OsmoGridUtils.buildNodeCodeMap(subGrid.getRawGrid.getNodes)
